...
# Plot 15: Delta vs PCA 1 and PCA 2
...
popt, pcov = curve_fit(fit_func, df["pca1"], df["delta"], bounds=([0, 0, -10000], [1000, 1000000, 10000]))
df['residuals1'] = df['delta'] - fit_func(df['pca1'], *popt)
popt, pcov = curve_fit(fit_func, df["pca2"], df["delta"], bounds=([0, 0, -10000], [1000, 1000000, 10000]))
df['residuals2'] = df['delta'] - fit_func(df['pca2'], *popt)
...
# # Plot 19: Residuals vs PC1 (individual graphs for each month)
# for month in df["Month"].unique():
#     plt.scatter(df[df["Month"] == month]["pca1"], df[df["Month"] == month]["residuals1"], label=f"Month {month}", s=10)
#     plt.xlabel(f"PC1 for {month}")
...
# # Plot 20: Residuals vs PC1 (superimposed graphs)
# plt.scatter(df["pca1"], df["residuals1"], label="Data")
# plt.xlabel("PC1 of Right Ascension and Declination")
# plt.ylabel("Residuals")
# plt.title("Residuals vs PC1")
# plt.legend()
# plt.show()

# Plot 21: Delta vs PC1 (superimposed graphs)
for cycle, df in lunar_dfs.items():
    plt.scatter(df["pca1"], df["delta"], label=f"Lunar Cycle {cycle}", s=10)
plt.xlabel("PC1")
plt.ylabel("Delta (AU)")
plt.title("Delta vs PC1")
plt.legend()
plt.show()

# Plot 22: Delta vs PC2 (superimposed graphs)
for cycle, df in lunar_dfs.items():
    plt.scatter(df["pca2"], df["delta"], label=f"Lunar Cycle {cycle}", s=10)
plt.xlabel("PC2")
plt.ylabel("Delta (AU)")
plt.title("Delta vs PC2")
plt.legend()
plt.show()

# Plot 23: Residuals vs PC1 (superimposed graphs)
for cycle, df in lunar_dfs.items():
    plt.scatter(df["pca1"], df["residuals1"], label=f"Lunar Cycle {cycle}", s=10)
plt.xlabel("PC1")
plt.ylabel("Residuals")
plt.title("Residuals vs PC1")
plt.legend()
...
# Plot 24: Residuals vs PC2 (superimposed graphs)
for cycle, df in lunar_dfs.items():
    plt.scatter(df["pca2"], df["residuals2"], label=f"Lunar Cycle {cycle}", s=10)
plt.xlabel("PC2")
plt.ylabel("Residuals")
plt.title("Residuals vs PC2")
plt.legend()
plt.show()