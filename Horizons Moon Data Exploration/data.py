...
## Make dataframe
header_list = [
    ["Datetime", "RA", "DEC", "Lon", "Lat", "APmag", "S-brt", "delta", "deldot", "S-O-T /r", "S-T-O", "Sky_motion", "Sky_mot_PA", "RelVel-ANG", "Lun_Sky_Brt", "sky_SNR"],
]
...
for line in data:
    ...
    Coord_ecliptic = Coord.transform_to("geocentrictrueecliptic") # convert to ecliptic coordinates
    lon = Coord_ecliptic.lon.degree
    lat = Coord_ecliptic.lat.degree
    APmag = float(line[8])
    S_brt = line[9]
    delta = float(line[10])
    ...
    Lun_Sky_Brt = line[18]
    sky_SNR = line[19]
    line = [date, RA, DEC, lon, lat, APmag, S_brt, delta, deldot, S_O_T, S_T_O, Sky_motion, Sky_mot_PA, RelVel_ANG, Lun_Sky_Brt, sky_SNR]
    df.loc[len(df)] = line
